...
function Steps() {
  ...
  const tref = useRef();
  const onScreen = useOnScreen(tref, "-30%");
  ...
  return (
    <>
      ...
      <div ref={tref} className="hidden md:flex flex-col h-screen w-full overflow-y-scroll overscroll-auto z-0  scrollbar snapping  snapped font-bold ">
        {
          onScreen ? (
            <>
      ...
      <m.div className=" flex md:hidden flex-col h-screen w-full overflow-y-scroll overscroll-auto z-0  scrollbar snapping  snapped font-bold ">
        <m.div className="h-full w-full">
          {list2.map(({ color, text1, text2, highlitedText }, index1) => {
            return (
              <m.div
